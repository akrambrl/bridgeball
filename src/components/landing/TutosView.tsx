...
function getTutos() {
  const principle = tr("Le principe.", "The concept.", "Das Prinzip.", "Il principio.", "O conceito.","La idea.");
  const scoring = tr("Le scoring.", "Scoring.", "Die Wertung.", "Il punteggio.", "A pontuação.","La puntuación.");
  const tip = tr("Astuce.", "Tip.", "Tipp.", "Consiglio.", "Dica.","Truco.");
  const howTo = (name: string) =>
    name + tr(" — Comment jouer", " — How to play", " — So spielt man", " — Come si gioca", " — Como jogar"," — Cómo jugar");
      ...
      content: (
        <div className="space-y-3 text-sm md:text-base leading-relaxed text-white/80">
          <P label={principle} text={tr(
            ...
            "Dois clubes aparecem (ex. Real Madrid e Milan). Ache um jogador que vestiu as duas camisas na carreira.","Aparecen dos clubes (por ejemplo Real Madrid y AC Milan). Encuentra un jugador que haya vestido las dos camisetas en su carrera.")} />
          <P label={scoring} text={tr(
            ...
            "Quanto mais rápido você responde, mais pontua. Uma resposta rara dá bônus. Encadeie combos para estourar seu total.","Cuanto antes aciertes, más puntúas. Una respuesta rara da bonus. Encadena combos para disparar tu total.")} />
          <P label={tip} text={tr(
            ...
            "Pense em jogadores que rodaram a Europa (Ibrahimović, Anelka, Materazzi). PSG, Real, Inter, Milan, grandes clubes portugueses → hubs de ouro.","Piensa en jugadores que dieron la vuelta a Europa (Ibrahimović, Anelka, Materazzi). PSG, Real, Inter, Milan, grandes clubes portugueses → puentes de oro.")} />
        </div>
      ),
    },
    {
      ...
      accent: G.projecteur,
      badge: tr("MARATHON", "MARATHON", "MARATHON", "MARATONA", "MARATONA","MARATÓN"),
      content: (
        <div className="space-y-3 text-sm md:text-base leading-relaxed text-white/80">
          <P label={principle} text={tr(
            ...
            "Você começa por um jogador. A cada turno, diga outro jogador que dividiu ao menos um clube com o anterior. A corrente cresce, transferência após transferência.","Partes de un jugador. En cada turno nombras a otro que haya compartido al menos un club con el anterior. La cadena crece, traspaso a traspaso.")} />
          <P label={scoring} text={tr(
            ...
            "Cada elo válido dá pontos. O jogo acaba no primeiro erro ou quando o tempo esgota.","Cada eslabón válido suma puntos. La partida termina al primer fallo o cuando se acaba el tiempo.")} />
          <P label={tip} text={tr(
            ...
            "Tenha em mente jogadores-hub (Zlatan, Cantona, Drogba). Veteranos de Arsenal e Chelsea são pérolas para esticar a corrente.","Ten en mente a los jugadores puente (Zlatan, Cantona, Drogba). Los veteranos del Arsenal y del Chelsea son oro para alargar la cadena.")} />
        </div>
      ),
    },
    {
      id: "grid",
      title: howTo(tr("Trouve le joueur", "Guess the Player", "Errate den Spieler", "Indovina il giocatore", "Adivinhe o jogador","Adivina el jugador")),
      accent: G.pelouse,
      badge: tr("ILLIMITÉ", "UNLIMITED", "UNBEGRENZT", "ILLIMITATO", "ILIMITADO","SIN LÍMITE"),
      content: (
        <div className="space-y-3 text-sm md:text-base leading-relaxed text-white/80">
          <P label={principle} text={tr(
            ...
            "Um jogador misterioso para adivinhar, ilimitado. A carreira fica escondida: você deduz só pelos seus palpites (revelável como dica).","Un jugador misterioso para adivinar, sin límite. Su trayectoria queda oculta: deduces solo a partir de tus propuestas (se puede revelar como pista).")} />
          <P label={scoring} text={tr(
            ...
            "Você tem 6 tentativas. Cada palpite mostra fichas: nacionalidade, zona, posição, idade (↑↓), clube e clubes em comum — ✓ verde, 🟨 perto, ✗ vermelho.","Tienes 6 intentos. Cada propuesta muestra fichas: nacionalidad, zona, posición, edad (↑↓), club y clubes en común — ✓ verde, 🟨 cerca, ✗ rojo.")} />
          <P label={tip} text={tr(
            ...
            "Encadeie acertos para subir sua SÉRIE 🔥: o ranking premia as melhores sequências. Desafie seus amigos com o enigma compartilhável.","Encadena aciertos para subir tu RACHA 🔥: la clasificación premia las mejores rachas. Reta a tus colegas con el enigma que puedes compartir.")} />
        </div>
      ),
    },
  ];
    ...
    <div className="container max-w-3xl mx-auto px-6 lg:px-10 py-10">
      <div className="text-center mb-8">
        <span className="inline-block px-3 py-1 mb-3"
          ...
          {tr("TUTORIELS", "TUTORIALS", "ANLEITUNGEN", "TUTORIAL", "TUTORIAIS","TUTORIALES")}
        </span>
        <h2 style={{ ...posterTitre(80, G.white), fontSize:"clamp(44px,8vw,80px)" }}>
          {tr("COMMENT JOUER", "HOW TO PLAY", "SO SPIELT MAN", "COME SI GIOCA", "COMO JOGAR","CÓMO JUGAR")}
        </h2>
        <p className="mt-4 text-white/60">
          {tr("3 minutes pour comprendre. Une vie pour devenir bon.", "3 minutes to understand. A lifetime to master.", "3 Minuten zum Verstehen. Ein Leben, um gut zu werden.", "3 minuti per capire. Una vita per diventare bravo.", "3 minutos para entender. Uma vida para dominar.","3 minutos para entenderlo. Una vida para dominarlo.")}
        </p>
      </div>
